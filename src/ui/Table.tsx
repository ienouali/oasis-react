...
function Header({children}: TableHeaderProps) {
    ...
    return <StyledHeader as='header' role='row' columns={columns}>
        {children}
    </StyledHeader>
}
...
function Body<A>({data, render}: TableBodyProps<A>) {
    if (!data.length) return <StyledEmpty />
